...
class Car:
    """Car object."""
    x: float = 400.0  # Horizontal position (center)
    y: float = 550.0  # Vertical position (near bottom)
...
class CarRaceGame:
    ...
    def __init__(self, screen_width: int = 800, screen_height: int = 600):
        ...
        # EEG control
        self.focus_threshold = 0.6  # Default threshold (0-1)
    ...
    def start_race(self):
        ...
        self.race_time = 0.0
        self.distance_traveled = 0.0
        # Reset car to starting position (bottom of screen)
        self.car = Car(x=400.0, y=550.0, speed=0.0, max_speed=8.0)
        self.focus_streak = 0
        self.max_focus_streak = 0
        self.current_focus = 0.0
        self.last_update = time.time()
    ...
    def update(self, delta_time: float):
        ...
        # Update car movement based on focus
        if self.current_focus >= self.focus_threshold:
            # Move forward - speed proportional to focus
            if self.focus_threshold < 1.0:
                focus_factor = (self.current_focus - self.focus_threshold) / (1.0 - self.focus_threshold)
            else:
                focus_factor = 1.0
            focus_factor = max(0.0, min(1.0, focus_factor))  # Clamp to 0-1
            self.car.speed = self.car.max_speed * focus_factor
        else:
            # Slow down or stop when focus is low
            self.car.speed *= 0.85  # Deceleration
            if self.car.speed < 0.1:
                self.car.speed = 0.0
        
        # Move car forward (up the screen)
        self.car.y -= self.car.speed * delta_time * 60
        ...
        # Keep car in bounds horizontally
